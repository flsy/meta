...
describe('isTruthy', () => {
  ...
  it('should work for numbers', () => {
    expect(validateField(getTextMeta({ name: 'Name', validation }), { Name: 0 })).toEqual('Looks falsy to me');
  ...
  describe('boolean value', () => {
    it('validates checkbox fields', () => {
      expect(validateField(getCheckboxMeta({ name: 'Checkbox', validation }), { Checkbox: null })).toEqual('Looks falsy to me');
      expect(validateField(getCheckboxMeta({ name: 'Checkbox', validation }), { Checkbox: false })).toEqual('Looks falsy to me');
      expect(validateField(getCheckboxMeta({ name: 'Checkbox', validation }), { Checkbox: true })).toEqual(undefined);
    });
  });
